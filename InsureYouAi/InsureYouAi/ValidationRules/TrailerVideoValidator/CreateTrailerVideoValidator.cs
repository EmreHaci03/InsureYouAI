...
{
    public class CreateTrailerVideoValidator : AbstractValidator<CreateTrailerVideoDto>
    {
        public CreateTrailerVideoValidator()
        {
            RuleFor(x => x.Title)
            ...
            RuleFor(x=>x.VideoUrl)
                .NotEmpty()
                .WithMessage("Video Yolu Boş Bırakılamaz")
                .Must(url=> ! string.IsNullOrEmpty(url) && (url.EndsWith(".jpg") || url.EndsWith(".jpeg") || url.EndsWith(".png")))
                .WithMessage("Geçersiz resim formatı. (jpg, jpeg, png desteklenir)");
